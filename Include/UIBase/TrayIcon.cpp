...
LRESULT CTrayIcon::OnTrayNotification(UINT wParam, LONG lParam) 
{
	...
	{	
		...
		if (!(pSubMenu = (BCMenu*)menu.GetSubMenu(0))) return 0;
		...
		GetCursorPos(&pos);
		//::SetForegroundWindow(m_tnd.hWnd);  
		::TrackPopupMenu(pSubMenu->m_hMenu, TPM_RIGHTALIGN | TPM_BOTTOMALIGN, pos.x, pos.y, 0, m_tnd.hWnd, NULL);
		menu.DestroyMenu();
	} 
