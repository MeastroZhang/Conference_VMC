...
class CBtnStatus : public CButton
{
public:
    CBtnStatus();
	~CBtnStatus();
...
public:
	...
	BOOL ModifyMenu( LPTSTR lpText,UINT nID,int nIconNormal );
#else
	DWORD SetMenu(UINT nMenu, HWND hParentWnd, BOOL bRepaint = TRUE);
#endif

	BOOL LoadImage(LPCTSTR lpFileName, int cx, int cy, int nIndex = 0);
	BOOL LoadImage(HBITMAP hBmpFirst, int cx, int cy, HBITMAP hBmpSec, int nIndex = 0);
	void SetShowImageIndex(int nIndex);
	void SetBtnFlage(BOOL bFlage);
...
#pragma pack()
	...
	CImage			m_ImageBK;	
	HDC				m_hSecBmpDc;
	int				m_nIndex;		// 需要刷的图片序号
	BOOL			m_bStatusFlage;	// 标识，是否通过图片列表刷图
	int				m_ncx;
	int				m_ncy;
	int				m_nSecBmpW;
	int				m_nSecBmpH;
