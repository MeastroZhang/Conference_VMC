...
BEGIN_MESSAGE_MAP(CBtnStatus, CButton)
    //{{AFX_MSG_MAP(CBtnStatus)
	ON_WM_SETCURSOR()
	ON_WM_KILLFOCUS()
	ON_WM_MOUSEMOVE()
...
void CBtnStatus::FreeResources(BOOL bCheckForNULL)
{
	if (bCheckForNULL)
	{
		// Destroy icons
...
void CBtnStatus::PreSubclassWindow() 
{
	UINT nBS;
...
BOOL CBtnStatus::PreTranslateMessage(MSG* pMsg) 
{
	InitToolTip();
...
LRESULT CBtnStatus::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
...
HBRUSH CBtnStatus::CtlColor(CDC* pDC, UINT nCtlColor) 
{
	return (HBRUSH)::GetStockObject(NULL_BRUSH); 
} // End of CtlColor

void CBtnStatus::OnSysColorChange() 
{
...
LRESULT CBtnStatus::OnSetStyle(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CBtnStatus::OnSetCheck(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CBtnStatus::OnGetCheck(WPARAM wParam, LPARAM lParam)
{
	ASSERT(m_bIsCheckBox);
	return GetCheck();
} // End of OnGetCheck
...
#ifdef	BTNST_USE_BCMENU
LRESULT CBtnStatus::OnMenuChar(UINT nChar, UINT nFlags, CMenu* pMenu) 
{
	LRESULT lResult;
...
#ifdef	BTNST_USE_BCMENU
void CBtnStatus::OnMeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct) 
{
	BOOL bSetFlag = FALSE;
...
void CBtnStatus::OnEnable(BOOL bEnable) 
{
...
void CBtnStatus::OnKillFocus(CWnd * pNewWnd)
{
	CButton::OnKillFocus(pNewWnd);
	CancelHover();
} // End of OnKillFocus

void CBtnStatus::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized) 
{
	CButton::OnActivate(nState, pWndOther, bMinimized);
	if (nState == WA_INACTIVE)	CancelHover();
} // End of OnActivate

void CBtnStatus::OnCancelMode() 
{
	CButton::OnCancelMode();
	CancelHover();
} // End of OnCancelMode

BOOL CBtnStatus::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
...
void CBtnStatus::OnMouseMove(UINT nFlags, CPoint point)
{
...
LRESULT CBtnStatus::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
	CancelHover();
	return 0;
} // End of OnMouseLeave

BOOL CBtnStatus::OnClicked() 
{	
	SetFocus();
...
void CBtnStatus::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	if (m_bStatusFlage)
	{
		...
		pt.x = m_nIndex*m_ncx;
		pt.y = (captionRect.Height() - m_ncy)/2;
		...
		::AlphaBlend( pDC->GetSafeHdc(), 5, pt.y, m_ncx, m_ncy, hDC, pt.x, 0, m_ncx, m_ncy, blend );
		...
		if ( m_hSecBmpDc )
		{
			CPoint ptSec;
			ptSec.x = 10+m_ncx;
			ptSec.y = (captionRect.Height() - m_nSecBmpH)/2;
			::AlphaBlend( pDC->GetSafeHdc(), ptSec.x, ptSec.y, m_nSecBmpW, m_nSecBmpH, m_hSecBmpDc, 0, 0, m_nSecBmpW, m_nSecBmpH, blend );
		}
...
void CBtnStatus::PaintBk(CDC* pDC)
{
...
HBITMAP CBtnStatus::CreateBitmapMask(HBITMAP hSourceBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTransColor)
{
	HBITMAP		hMask		= NULL;
	HDC			hdcSrc		= NULL;
	HDC			hdcDest		= NULL;
...
//
void CBtnStatus::PrepareImageRect(BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, DWORD dwWidth, DWORD dwHeight, CRect* rpImage)
{
	CRect rBtn;
...
void CBtnStatus::DrawTheIcon(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, BOOL bIsDisabled)
{
	BYTE		byIndex		= 0;
...
void CBtnStatus::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rItem, CRect *rCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	HDC			hdcBmpMem	= NULL;
...
//
HICON CBtnStatus::CreateGrayscaleIcon(HICON hIcon)
{
...
//
DWORD CBtnStatus::SetIcon(int nIconIn, int nIconOut)
{
	HICON		hIconIn			= NULL;
	HICON		hIconOut		= NULL;
...
//			Failed loading the specified resource.
//
DWORD CBtnStatus::SetIcon(HICON hIconIn, HICON hIconOut)
{
	BOOL		bRetValue;
	ICONINFO	ii;
...
//
DWORD CBtnStatus::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut)
{
...
//			Failed creating mask bitmap.
//
DWORD CBtnStatus::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut)
{
	int		nRetValue;
	BITMAP	csBitmapSize;
...
//				By default, CBtnStatus buttons are flat.
//		[IN]	bRepaint
...
//			Function executed successfully.
//
DWORD CBtnStatus::SetFlat(BOOL bFlat, BOOL bRepaint)
{
	m_bIsFlat = bFlat;
...
//				By default, CBtnStatus buttons have ST_ALIGN_HORIZ alignment.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
//
DWORD CBtnStatus::SetAlign(BYTE byAlign, BOOL bRepaint)
{
	switch (byAlign)
	{    
		case ST_ALIGN_HORIZ:
...
//			Function executed successfully.
//
DWORD CBtnStatus::SetCheck(int nCheck, BOOL bRepaint)
{
	if (m_bIsCheckBox)
	{
...
//			Function executed successfully.
//
DWORD CBtnStatus::SetDefaultColors(BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CBtnStatus::SetColor(BYTE byColorIndex, COLORREF crColor, BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CBtnStatus::GetColor(BYTE byColorIndex, COLORREF* crpColor)
{
	if (byColorIndex >= BTNST_MAX_COLORS)	return BTNST_INVALIDINDEX;
...
//
DWORD CBtnStatus::OffsetColor(BYTE byColorIndex, short shOffset, BOOL bRepaint)
{
	BYTE	byRed = 0;
	BYTE	byGreen = 0;
	BYTE	byBlue = 0;
...
//			Function executed successfully.
//
DWORD CBtnStatus::SetAlwaysTrack(BOOL bAlwaysTrack)
{
...
//			Failed loading the specified resource.
//
DWORD CBtnStatus::SetBtnCursor(int nCursorId, BOOL bRepaint)
{
	HINSTANCE	hInstResource = NULL;
...
//			Function executed successfully.
//
DWORD CBtnStatus::DrawBorder(BOOL bDrawBorder, BOOL bRepaint)
{
...
//			Function executed successfully.
//
DWORD CBtnStatus::DrawFlatFocus(BOOL bDrawFlatFocus, BOOL bRepaint)
{
...
//
void CBtnStatus::SetTooltipText(int nText, BOOL bActivate)
{
	CString sText;
...
//				If TRUE the tooltip will be created active.
//
void CBtnStatus::SetTooltipText(LPCTSTR lpszText, BOOL bActivate)
{
...
//
void CBtnStatus::ActivateTooltip(BOOL bActivate)
{
...
//				If TRUE the control will be repainted.
//
void CBtnStatus::DrawTransparent(BOOL bRepaint)
{
...
//			Function executed successfully.
//
DWORD CBtnStatus::SetURL(LPCTSTR lpszURL)
{
...
//
#ifndef	BTNST_USE_BCMENU
DWORD CBtnStatus::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bRepaint)
{
	HINSTANCE	hInstResource	= NULL;
...
//
#ifdef	BTNST_USE_BCMENU
DWORD CBtnStatus::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bWinXPStyle, UINT nToolbarID, CSize sizeToolbarIcon, COLORREF crToolbarBk, BOOL bRepaint)
{
	BOOL	bRetValue = FALSE;
...
BOOL CBtnStatus::ModifyMenu( LPTSTR lpText,UINT nID,int nIconNormal )
{
	//	return m_menuPopup.ModifyMenu(nPosition, nFlags, nIDNewItem, lpszNewItem);
	return m_menuPopup.ModifyODMenuW( lpText,nID,nIconNormal);
}

#endif

BOOL CBtnStatus::LoadImage(LPCTSTR lpFileName, int cx, int cy, int nIndex /*= 0*/)
{
	HRESULT rt = S_FALSE;
	
	m_ImageBK.Destroy();

	rt = m_ImageBK.Load(lpFileName);
...
BOOL CBtnStatus::LoadImage(HBITMAP hBmpFirst, int cx, int cy, HBITMAP hBmpSec, int nIndex /*= 0*/)
{
	ASSERT( hBmpFirst && hBmpSec );
	if ( !hBmpFirst || !hBmpSec )
	{
		return FALSE;
	}

	m_ImageBK.Destroy();
	m_ImageBK.Attach( hBmpFirst );
	
	BITMAP bmpSec;
	::GetObject( hBmpSec, sizeof(BITMAP), &bmpSec );
	CClientDC dc(this);
	m_hSecBmpDc = ::CreateCompatibleDC( dc.GetSafeHdc() );
	::SelectObject( m_hSecBmpDc, hBmpSec );
	
	m_nIndex = nIndex;
	m_ncx = cx;
	m_ncy = cy;
	m_nSecBmpW = bmpSec.bmWidth;
	m_nSecBmpH = bmpSec.bmHeight;

	return TRUE;
}

void CBtnStatus::SetShowImageIndex(int nIndex)
{
	m_nIndex = nIndex;
}

void CBtnStatus::SetBtnFlage(BOOL bFlage)
{
	m_bStatusFlage = bFlage;
}


// This function is called every time the button background needs to be painted.
// If the button is in transparent mode this function will NOT be called.
// This is a virtual function that can be rewritten in CBtnStatus-derived classes
...
//			Function executed successfully.
//
DWORD CBtnStatus::OnDrawBackground(CDC* pDC, LPCRECT pRect)
{
	COLORREF	crColor;
...
//			Function executed successfully.
//
DWORD CBtnStatus::OnDrawBorder(CDC* pDC, LPCRECT pRect)
{
	if (m_bIsPressed)
