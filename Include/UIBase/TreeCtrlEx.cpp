...
{
#if _MFC_VER < 0x0700
	return CreateEx( dwExStyle | LVS_OWNERDRAWFIXED, WC_TREEVIEW, NULL, dwStyle,
		rect.left, rect.top, rect.right-rect.left, rect.bottom-rect.top, 
		pParentWnd->GetSafeHwnd(), (HMENU)nID );
#else
	return CTreeCtrl::CreateEx( dwExStyle | LVS_OWNERDRAWFIXED, dwStyle, rect, pParentWnd, nID );
#endif
}

BOOL CTreeCtrlEx::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
{
	return CTreeCtrl::Create(dwStyle | LVS_OWNERDRAWFIXED, rect, pParentWnd, nID);
}
...
void CTreeCtrlEx::OnPaint() 
{
	...
	CBitmap   *   old;   
	CDC   dc1;   //   device   context   for   painting   
	CDC   mem;   
	...
	{		
			...
			&& ::GetFocus() == m_hWnd)
		{
			...
			HTREEITEM  currentitem = GetParentItem(hItem);
			HTREEITEM  rootitem = GetRootItem();
			HTREEITEM  currentChild = GetChildItem(hItem);
			int ustatud,ustatud1;
			GetItemImage(hItem,ustatud1,ustatud);
			strDowrd.Format(_T("%d"),ustatud);
			if((currentitem != 0)&&(m_iSetItemMulImageCnt == 2)&&((rootitem != currentitem)||((rootitem==currentitem)&&(m_bSetDepartImgFlg))))//&&(currentChild == 0) //del zlf 2009.03.19
			{
				int iLeft,iRight,iLength,istrlen;
				CString strLeft;
				CString strRight;

				strRight = strDowrd.Right(2);
				wsprintf(tchar,_T("%s"),strRight);
				iRight = _wtoi(tchar);

				iLength = strDowrd.GetLength();
				strLeft = strDowrd.Left(iLength - 2);

				wsprintf(tchar,_T("%s"),strLeft);
				iLeft = _wtoi(tchar);	
				GetListImage(m_ilTree,bitmap1,iLeft);
				GetListImage(m_ilTree,bitmap2,iRight);
				GetListImage(m_ilTree,bitmap3,ustatud1);

				mem.CreateCompatibleDC(&dc1);

				old=mem.SelectObject(&bitmap3);   
				memDC.BitBlt(rect.left-m_icx,rect.top ,m_icx,m_icx,&mem,0,0,SRCCOPY);  



				old=mem.SelectObject(&bitmap1);  

				wsprintf(tchar,_T("%s"),sItem);
				DWORD dwNum = WideCharToMultiByte(CP_OEMCP,NULL,tchar,-1,NULL,0,NULL,FALSE);

				WideCharToMultiByte (CP_OEMCP,NULL,tchar,-1,ch,dwNum,NULL,FALSE);

				istrlen = strlen(ch);

				//TConferenceUserInfo* pSelectItemInfo=(TConferenceUserInfo*)(GetItemData(hItem));
				//if (((TInfo*)pSelectItemInfo)->_InfoType != ntMCU  && pSelectItemInfo)
				{
					CString strMystr = tchar;
					if ( 0 == strMystr.Find(_T("Second")))
					{
						int aa = 0;
					}
					CSize MySize = memDC.GetTextExtent(strMystr);
					istrlen = MySize.cx;

					CSize TempSize = memDC.GetTextExtent(_T(" "));

					memset(ch,'\0',sizeof(char));
					memDC.BitBlt(rect.left + istrlen-(TempSize.cx*5),rect.top ,m_icx,m_icx,&mem,0,0,SRCCOPY);   
					old=mem.SelectObject(&bitmap2);   
					memDC.BitBlt(rect.left + istrlen-(TempSize.cx*5)+m_icx,rect.top ,m_icx,m_icx,&mem,0,0,SRCCOPY);  
				}
			...
			else if((currentitem != 0)&&(m_iSetItemMulImageCnt == 1)&&((rootitem != currentitem)||((rootitem==currentitem)&&(m_bSetDepartImgFlg)))&&(ustatud1 != ustatud))//&&(currentChild == 0) //del zlf 2009.03.19
			{
				int istrlen;

				GetListImage(m_ilTree,bitmap1,ustatud);
				GetListImage(m_ilTree,bitmap3,ustatud1);
				...
				//memset(ch,'\0',sizeof(char));
				//memDC.BitBlt(rect.left + istrlen*m_iFontSize + 3*m_icx,rect.top ,m_icx,m_icx,&mem,0,0,SRCCOPY);   
...
/*				memset(ch,'\0',sizeof(char));
				memDC.BitBlt(rect.left + istrlen*m_iFontSize + 3*m_icx,rect.top ,m_icx,m_icx,&mem,0,0,SRCCOPY);   
				old=mem.SelectObject(&bitmap2);   
				memDC.BitBlt(rect.left + istrlen*m_iFontSize + 3*m_icx + m_icx,rect.top ,m_icx,m_icx,&mem,0,0,SRCCOPY);*/  
...
void CTreeCtrlEx::GetListImage(CImageList &ImageList,CBitmap &Bitmap ,int nImageIdx)
{   
	...
	dcMemory.FillSolidRect(0, 0, nWidth, nHeight, m_ColcorMask);
...
void CTreeCtrlEx::SetTreeItemImage(CString strbmp,int cx,int cy,int nGrow,COLORREF colormask,int nPointSize)
{

	m_icx = cx;
	m_icy = cy;
	m_ColcorMask = colormask;
	m_iFontSize = nPointSize/20;
	m_Bitmap.DeleteObject();
	
	if (strbmp.Right(4) == _T(".bmp")) 
	{
		m_Bitmap.Attach(LoadImage(NULL, strbmp, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE)); 
	}
	else
	{
		if( PathFileExists(strbmp)) 
		{
			CImage imgTemp;
			imgTemp.Load(strbmp);
			m_Bitmap.Attach(imgTemp.Detach());
		}
	}

	m_ilTree.Create(cx,cy,ILC_COLOR32,0,nGrow);

	m_ilTree.Add(&m_Bitmap,colormask);
	SetImageList(&m_ilTree, TVSIL_NORMAL);

}

void CTreeCtrlEx::SetTreeItemMulImage(int iImageCnt)
{

	m_iSetItemMulImageCnt = iImageCnt;
	if(m_iSetItemMulImageCnt > 2)
	{
		m_iSetItemMulImageCnt = 2;
	}
}
void CTreeCtrlEx::SetTreeItemDepartImage(BOOL btrue)
{
	m_bSetDepartImgFlg = btrue;
}
...
void CTreeCtrlEx::MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
