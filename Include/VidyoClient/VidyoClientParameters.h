...
typedef enum VidyoClientMediaControlType_
{
	...
	VIDYO_CLIENT_MEDIA_CONTROL_TYPE_AUDIO,          /*!< Audio steam */
...
typedef struct VidyoClientAccountList_
{
	VidyoUint total;
	VidyoClientAccountData *accountData;
} VidyoClientAccountList;


typedef struct VidyoClientParticipantData_
{
	/*! Entity ID - Unique in Portal DB */
	char entityID[MAX_EID_SIZE];// Entity ID - Unique in Portal DB
	/*! Participant ID is unique per conference participant , Valid only when user is in conference*/
	...
	VidyoBool appShare;
	/*! handRaised */
	VidyoBool handRaised;
	/*! presenter */
	VidyoBool presenter;
} VidyoClientParticipantData;

typedef struct VidyoClientParticipantList_
{
	VidyoUint total;
	VidyoBool lectureMode;
	int		  RecordID;
	VidyoClientParticipantData *participantData;
} VidyoClientParticipantList;
...
/*! Event structure that is notified back to user as part of out event Service for joining conference
...
/*! Event structure that is notified back to user as part of out event Service for search request
	...
	- Used for the following events:
		+ #VIDYO_CLIENT_OUT_EVENT_PORTAL_SERVICE / VVIDYO_CLIENT_SERVICE_JOIN_CONFERENCE
		+ #VidyoClientOutEventPortalService
*/
